...
const sourcePath = path.join(__dirname, 'SBMS-System-Verification-Playbook.md');
const outputPath = path.join(__dirname, 'SBMS-System-Verification-Playbook.pdf');
const aliasOutputPath = path.join(__dirname, 'playbook.pdf');
const stdoutMode = process.argv.includes('--stdout-base64');
...
function drawHeader() {
  ...
  doc.setFontSize(16);
  doc.text('SBMS System Verification Playbook', left, 27);
...
if (stdoutMode) {
  ...
  fs.writeFileSync(aliasOutputPath, pdfBuffer);
  console.log(`Generated: ${outputPath}`);
  console.log(`Updated alias: ${aliasOutputPath}`);
}
