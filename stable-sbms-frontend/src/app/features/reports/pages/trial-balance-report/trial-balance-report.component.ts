...
import { GlJournalResponse, ReportRequestLogResponse, TrialBalanceResponse } from '../../models/report.model';
import { ReportService } from '../../services/report.service';

type TrialBalanceExportType = 'VIEW' | 'CSV' | 'EXCEL' | 'PRINT';
...
export class TrialBalanceReportComponent implements OnInit {
  ...
  activeExportType: TrialBalanceExportType = 'VIEW';
  ...
  loadReport(): void {
    this.loading = true;
    forkJoin({
      report: this.reportService.getTrialBalance(this.filters),
      ...
      error: err => {
        ...
        this.exportHistory = [];
        this.loading = false;
        Swal.fire('Error', 'Failed to load trial balance.', 'error');
      }
    });
  }
  ...
  onExport(type: TrialBalanceExportType): void {
    if (type === 'VIEW') {
      this.loadReport();
      return;
    }
  ...
  openLedgerProfitLoss(): void {
    window.location.href = '/reports/ledger-profit-loss';
  }

  openHistory(): void {
    const el = document.getElementById('trialBalanceExportHistory');
    if (el) {
  ...
  downloadHistoryFile(id: number): void {
    this.reportService.downloadExportHistoryFile(id).subscribe({
      next: res => this.saveBlob(res.body || null, this.extractFileName(res.headers.get('content-disposition')) || 'trial-balance'),
      error: err => {
        console.error(err);
  ...
  get controlDifference(): number {
    return Math.abs((this.report?.totalDebit || 0) - (this.report?.totalCredit || 0));
  }

  get controlStatusLabel(): string {
    return this.controlDifference === 0 ? 'Balanced' : 'Mismatch';
  }
  ...
  private generatePersistentRun(exportType: 'CSV' | 'EXCEL' | 'PRINT' | null, action: 'preview' | 'print' | 'download'): void {
    this.reportService.getTrialBalanceReport({
      dateFrom: this.filters.dateFrom,
      dateTo: this.filters.dateTo,
      branchId: this.filters.branchId,
      exportType,
      ...
      error: err => {
        console.error(err);
        Swal.fire('Error', 'Failed to generate trial balance file.', 'error');
      }
    });
  }
  ...
  private loadJournals(filters: {
    ...
    this.reportService.getLedgerJournals(filters).subscribe({
      next: data => {
        ...
        const el = document.getElementById('trialBalanceJournalDrilldown');
        if (el) {
