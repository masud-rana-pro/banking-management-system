...
export class SuspiciousActivityViewComponent implements OnInit {
  ...
  load(id: number): void {
    this.loading = true;
    this.securityService.getSuspiciousActivityById(id).subscribe({
      next: data => {
        this.item = data;
        this.loading = false;
      },
      error: err => {
        console.error(err);
        this.loading = false;
        Swal.fire('Error', 'Failed to load suspicious activity detail.', 'error');
      }
    });
  }
